...
1. **Missing upstream data** — The RNAsum pipeline requires Dragen WGTS RNA, Sash, and Arriba WGTS RNA SUCCEEDED runs for the same libraries. Check that all upstream runs have completed successfully.
...
- Verify that input URIs (dragenWgtsRnaDir, sashDir, arribaWgtsRnaDir) are accessible
