...
CODE_VERSION="35c78cd"
PAYLOAD_VERSION="2025.09.30"
...
The output uri prefix and logs uri prefixes must be set to a location inside the s3 prefix that the ICA project is mounted on.
...
Example usage:
bash generate-WRU-draft.sh tumor_dna_library_id normal_dna_library_id tumor_rna_library_id \\
  --comment 'Redriving analysis after failure'

bash generate-WRU-draft.sh tumor_dna_library_id normal_dna_library_id tumor_rna_library_id \\
  --comment 'Redriving analysis after failure' \\
  --output-uri-prefix s3://project-bucket/analysis/rnasum/ \\
  --logs-uri-prefix s3://project-bucket/logs/rnasum/ \\
...
# Check that DATASET is within the AVAILABLE_DATASETS_ARRAY
if [[ ! " ${AVAILABLE_DATASETS_ARRAY[*]} " =~ " ${DATASET} " ]]; then
  # whatever you want to do when array doesn't contain value...
  echo_stderr "Got '${DATASET}' for --dataset parameter, invalid dataset"
  print_usage
  exit 1
fi
...
elif [[ "$(jq 'length' <<< "${libraries}")" -lt 3 ]]; then
  echo_stderr "Error: Less than 3 linked libraries found for the provided library IDs. At least a tumor dna, normal dna and tumor rna library is required. Exiting."
  exit 1
else
