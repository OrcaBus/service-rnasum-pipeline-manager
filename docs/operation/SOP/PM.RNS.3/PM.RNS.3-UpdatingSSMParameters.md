...
From time-to-time there may be a requirement to add or update SSM pipeline parameters for the RNAsum pipeline. This SOP describes how to modify SSM parameters that control pipeline behaviour (project IDs, output prefixes, reference data paths, workflow versions, etc.).
...
- `DEFAULT_WORKFLOW_VERSION` — the default rnasum workflow version
...
2. Follow the [Manual Pipeline Execution SOP](../PM.RNS.1/PM.RNS.1-ManualPipelineExecution.md) to verify changes are working
3. Run the [Workflow Validations SOP](../PM.RNS.4/PM.RNS.4-RunningWorkflowValidations.md) for comprehensive testing
