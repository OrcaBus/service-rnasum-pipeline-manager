...
def get_sash_data(
    ...
    output_relative_path = Path(latest_payload_data['outputs']['sashRelPath'])

    # Get the following files as outputs
    pcgr_tiers_tsv = output_relative_path / "smlv_somatic" / "report" / "pcgr" / ( latest_payload_data['inputs']['tumorDnaSampleId'] + ".pcgr_acmg.grch38.snvs_indels.tiers.tsv")
    purple_gene_tsv = output_relative_path / "purple" / (latest_payload_data['inputs']['tumorDnaSampleId'] + ".purple.cnv.gene.tsv")
    sv_tsv = output_relative_path / "sv_somatic" / "prioritise" / (latest_payload_data['inputs']['tumorDnaSampleId'] + ".sv.prioritised.tsv")

    return {
        "pcgrTiersTsv": extend_s3_uri_path(portal_run_id_analysis_root_prefix, str(pcgr_tiers_tsv)),
        "purpleGeneTsv": extend_s3_uri_path(portal_run_id_analysis_root_prefix, str(purple_gene_tsv)),
        "svTsv": extend_s3_uri_path(portal_run_id_analysis_root_prefix, str(sv_tsv)),
    }
